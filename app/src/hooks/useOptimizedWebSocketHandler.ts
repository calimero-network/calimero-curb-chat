...
export function useOptimizedWebSocketHandler(
  ...
  const processBatch = useCallback(async () => {
    ...
    try {
      ...
      if (batchedCount > 0 && finalConfig.enableMetrics) {
        log.debug('WebSocketHandler', `Coalesced ${batchedCount} redundant events`);
      ...
      if (finalConfig.enableMetrics) {
        ...
        if (processTime > 100) {
          log.warn('WebSocketHandler', `Slow batch processing: ${processTime.toFixed(2)}ms for ${eventsToProcess.length} events`);
        }
      }
    } catch (error) {
  ...
  const handleEvent = useCallback((event: WebSocketEvent) => {
    ...
    const shouldProcessNow = eventBatchRef.current.length >= finalConfig.maxBatchSize;
