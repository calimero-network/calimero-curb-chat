...
export function useAppNotifications(currentChatId?: string) {
  ...
  const { playSoundForMessage, playSound, isEnabled: soundEnabled } = useNotificationSound(
    {
      enabled: true,
      volume: 0.5,
      respectFocus: true,
  ...
  const notify = useCallback((notification: AppNotification) => {
    const { title, message, type = 'message', duration = 5000, playSound: shouldPlaySound = true } = notification;

    // Determine variant based on notification type
    const toastVariant = type === 'mention' ? 'warning' : type === 'dm' ? 'info' : 'info';
    const notificationVariant = type === 'mention' ? 'warning' : 'info';
    const priority = type === 'mention' ? 'high' : type === 'dm' ? 'medium' : 'low';
   ...
   */
  const notifyMessage = useCallback((messageId: string, sender: string, text: string, isMention: boolean = false) => {
    const type: NotificationType = isMention ? 'mention' : 'message';
    const title = isMention ? `${sender} mentioned you` : `New message from ${sender}`;
   ...
   * Notify for a new DM
   */
  const notifyDM = useCallback((messageId: string, sender: string, text: string) => {
    const truncatedText = text.length > 100 ? `${text.substring(0, 100)}...` : text;
   ...
   */
  const notifyChannel = useCallback((messageId: string, channelName: string, sender: string, text: string) => {
    const truncatedText = text.length > 100 ? `${text.substring(0, 100)}...` : text;
