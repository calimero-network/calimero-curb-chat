...
export default function CreateIdentityTab() {
  ...
  const handleCreateIdentity = async () => {
    ...
    try {
      const response: ResponseData<NodeIdentity> = await apiClient.node().createNewIdentity();

      if (response.error) {
        setMessage({ text: response.error.message || "Failed to create identity", type: "error" });
      } else if (response.data) {
        setIdentity(response.data);
        // Auto-save to localStorage
        localStorage.setItem("new-context-identity", JSON.stringify(response.data));
        setMessage({ text: "Identity created and saved successfully!", type: "success" });
      }
    } catch (_error) {
      setMessage({ text: "An error occurred while creating identity", type: "error" });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      {identity && (
        <IdentityDisplay>
          ...
          <InputGroup>
            ...
            <IdentityValue>
              {identity.publicKey}
              <CopyButton onClick={() => handleCopyToClipboard(identity.publicKey)}>
                Copy
              </CopyButton>
            </IdentityValue>
          </InputGroup>
          ...
          {identity.privateKey && (
            <InputGroup>
              ...
              <IdentityValue>
                {identity.privateKey}
                <CopyButton onClick={() => handleCopyToClipboard(identity.privateKey)}>
                  Copy
                </CopyButton>
              </IdentityValue>
            </InputGroup>
