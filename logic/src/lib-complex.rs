...
impl CurbApp {
    ...
    pub fn join(&mut self) -> Result<(), String> {
        ...
        let _ = self
            .member_channels
            .insert(executor_id, UnorderedSet::new());
        Ok(())
    }
    ...
    fn validate_group_name(&self, name: &String) {
        if name.len() == 0 {
    ...
    fn internal_create_group(
        ...
        self.validate_group_name(&group.name);
        ...
        let _ = self
            .channel_members
            .insert(group.clone(), UnorderedSet::new());
    ...
    fn group_invite(&mut self, group: Channel, account: UserId, self_join: Option<bool>) {
        ...
        if let Ok(Some(mut member_channels)) = self.member_channels.get(&account) {
            ...
            let _ = self
                .member_channels
                .insert(account.clone(), member_channels);
        }
    ...
    pub fn send_message(
        ...
        let message_id = Curb::get_message_id(&executor_id, &account, &group, &message, timestamp);
    ...
    pub fn get_messages(&self, group: Channel) -> FullMessageResponse {
